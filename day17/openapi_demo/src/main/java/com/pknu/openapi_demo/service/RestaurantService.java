...
public class RestaurantService {
    ...
    public GetFoodKr fetchRestaurant(int pageNo) {
        ...
        try {
            ...
            GetFoodKr rtFoodKr = reponse.getFoodKr;
            return rtFoodKr;
    ...
    public int saveRestaurantToDB(int pageNo) {
        GetFoodKr rtFoodKr = fetchRestaurant(pageNo);
        ...
        for (Item item : foodList) {
            if(restaurantRepository.existsById(item.UC_SEQ)){ // 중복된 키가 없으면
                ...
                restaurantRepository.save(entity); // DB에 저장
                saveCount++;
            }
        }
        return saveCount;
