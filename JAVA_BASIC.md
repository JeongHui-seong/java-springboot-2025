...
- 연산자는 거의 대부분의 언어에서 동일하게 사용
    ...
    - [Java 실습](./Day01/spring01/src/main/java/com/jhs/spring01/Spring01Application.java)
...
- 조건(분기)문: if문, switch문
    ...
    - [Java 실습](./Day01/spring02/src/main/java/com/jhs/spring02/Spring02Application.java)
...
- 반복문 : for문, while문, do-while문
    ...
    - [Java 실습](./Day01/spring03/src/main/java/com/jhs/spring03/Spring03Application.java)
...
- 반복제어 키워드 : break, continue
    ...
    - [Java 실습](./Day02/spring01/src/main/java/com/jhs/spring01/Spring01Application.java)
...
- 배열에 사용할 메서드: Arrays 객체 사용
    - sort(): 배열 정렬
    - toString(): 배열 객체ID 출력
    - asList(): 배열을 리스트로 변경(List)
    - copyOf(): 배열을 복사해서 새로운 배열로 반환
    - equals(): 두 개의 배열이 동일한지 여부 판별
    - binarySearch(): 배열내 요소를 찾아서 인덱스 반환

- Java 컬렉션 프레임워크: 배열을 기능적으로 강화한 객체
    - ArrayList, Stack, Vector, **List**, HashSet, HashMap(파이썬의 딕셔너리), HashTable

    - [Java 실습](./Day02/spring02/src/main/java/com/jhs/spring02/Spring02Application.java)
...
- **null**
    ...
    - [Java 실습](./Day02/spring03/src/main/java/com/jhs/spring03/Spring03Application.java)
...
- 객체 개요
    ...
    - 반대. Procedure-Oriented Programming - 절차적 프로그래밍
        - C, SQL, ...
...
- 인스턴스(instance)
    ...
    - [Java 실습](./Day02/spring04/src/main/java/com/jhs/spring04/Spring04Application.java)
...
- 특징
    ...
    - 동일한 부모클래스를 상속받은 하위 자식클래스들은 타입이 호환됨
    ...
    - [Java 실습](./Day02/spring05/src/main/java/com/jhs/spring05/Spring05Application.java)

#### 접근제어자
- 클래스, 변수, 메서드 등에 외부에서 어떤 방식으로 접근할건지 정의하는 키워드
...
- 아무런 기능이 없고 단지 어떻게 구현을 해야 한다는 규칙만 선언
...
- 스프링의 의존선 주입(Dependency Injection)의 핵심

- [Java 실습](./Day03/spring01/src/main/java/com/jhs/spring01/Spring01Application.java)

#### 예외처리
- 프로그램의 비정상적 종료를 막기 위한 보호막
- 소스코드 상의 오류: 에러/컴파일 에러, 수정이 용이
- 실행 중 발생하는 오류: 예외/런타임 에러, 수정이 어려움, 언제 발생할지 모름
...
##### 예외던지기
- 예외를 직접 차리하지 않고 상위 클래스나 메서드에게 처리를 인가
...
- [Java 실습](./Day03/spring02/src/main/java/com/jhs/spring02/Spring02Application.java)
...
- [Java 실습](./Day03/spring04/src/main/java/com/jhs/spring04/Spring04Application.java)
...
-[Java 실습](./Day04/spring01/src/main/java/com/jhs/spring01/Spring01Application.java)
...
-[Java 실습](./Day04/spring02/src/main/java/com/jhs/spring02/Spring02Application.java)
...
-[Java 실습](./Day04/spring03/src/main/java/com/jhs/spring03/Spring03Application.java)
...
- 스트림API가 쉽지는 않지만, 코딩량을 줄일 수 있음.

- [Java 실습](./Day04/spring04/src/main/java/com/jhs/spring04/Spring04Application.java)

#### StringBuilder
- String 객체는 불변의 성질, 한 번 생성되면 변경 불가
- 연산자로 수정을 하면 기존 메모리는 두고, 다시 String 객체를 생성
- 이를 해결하고자 하기 위해 만든 것 - StringBuilder
- 리스트와 유사, 문자열 처리때문에 메모리 문제, 성능 문제가 발생하면 StringBuilder, StringBuffer 사용 고려
...
- [Java 실습](./Day05/spring01/src/main/java/com/jhs/spring01/Spring01Application.java)

#### GC(Garbage Collection)
- C등 절차적 언어에서는 객체를 생성해서 메모리 사용하면, 개발자가 직접 해제해줘야 함
- 메모리 누수가 발생
- 객체지향 언어에서는 GC 만들어서 메모리를 언어가 직접 핸들링 처리

#### File IO (파일 입출력)
- 파일을 읽고 쓰는 작업
- 파일 쓰기에 `FileOutputStream`, `FileWriter`, `PrintWriter` 등의 클래스 사용
- 입출력 예외가 발생할 수 있기 때문에 클래스나 메서드에 `throws IOException`을 추가해야함
- 파일 읽기에 `FileInputStream`, `BufferedReader` 클래스 사용

- [Java 실습](./Day05/spring02/src/main/java/com/jhs/spring02/Spring02Application.java)

#### 어노테이션
- 한글로 주석이지만, #, //, /**/과 같이 소스에 영향을 미치지 않는 주석과 다름
- 자바 소스에 추가하여 여러가지 기능을 수행하는 메타데이터의 일종
- @로 시작, JDK 1.5 이상부터 사용가능
- 클래스 파일에 같이 포함되어 JVM 작동시 실행됨
- 클래스, 메서드 바로 위에 작성, 코드와 설정을 관리할 수 있게 도와주는 역할

##### 1. @Override
- 오버라이드를 올바르게 했는지 컴퍼일러 체크
- 상속, 인터페이스 구현시 사용

##### 2. @Deprecated
- 앞으로 다음 버전에서 삭제될 수 있음, 사용하지 말 것을 권유하는 체크
<img src = "./image/sb0008.png">
